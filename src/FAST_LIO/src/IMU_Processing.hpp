...
class ImuProcess
{
 ...
 private:
  ...
  M3D Lidar_R_wrt_IMU; // 雷达相对imu的旋转外参
  V3D Lidar_T_wrt_IMU; // 雷达相对imu的平移外参
  V3D mean_acc;
  V3D mean_gyr;
  V3D angvel_last;
  V3D acc_s_last;
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  auto it_pcl = pcl_out.points.end() - 1; // 点云迭代器
  // 反向遍历IMU位姿序列，对点云进行运动补偿
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)
  {
    ...
    for(; it_pcl->curvature / double(1000) > head->offset_time; it_pcl --)
    {
      ...
      /* 核心：点云坐标变换与运动补偿
       * 补偿思路：将采集时刻t的点云坐标，转换到雷达帧结束时刻的坐标（统一坐标系）
       * 变换公式：P_compensate = R_imu_e^T * (R_i * P_i + T_ei)
       * 其中：R_i为采集时刻旋转矩阵，T_ei为采集时刻到结束时刻的平移量，R_imu_e为结束时刻旋转矩阵
       */
      M3D R_i(R_imu * Exp(angvel_avr, dt));  // 计算采集时刻t的旋转矩阵(通过角速度积分)
