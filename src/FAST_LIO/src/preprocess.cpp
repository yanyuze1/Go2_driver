...
void Preprocess::avia_handler(const livox_ros_driver2::msg::CustomMsg::UniquePtr &msg)
{
  ...
  if (feature_enabled)
  {
    for (uint i = 1; i < plsize; i++)
    {
      ...
      {
        const double angle_deg = -10.0;
        const double angle_rad = angle_deg * M_PI / 180.0;
        double cos_theta = cos(angle_rad);
        double sin_theta = sin(angle_rad);
        pl_full[i].x = msg->points[i].x * cos_theta - msg->points[i].z * sin_theta;
        pl_full[i].y = msg->points[i].y;
        pl_full[i].z = msg ->points[i].x * sin_theta + msg->points[i].z * cos_theta;
...
void Preprocess::velodyne_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  else
  {
    for (int i = 0; i < plsize; i++)
    {
      ...
      added_pt.normal_y = 0;
      added_pt.normal_z = 0;
      added_pt.x = pl_orig.points[i].x * cos_theta - pl_orig.points[i].z * sin_theta;
      added_pt.y = pl_orig.points[i].y;
      added_pt.z = pl_orig.points[i].z * cos_theta + pl_orig.points[i].x * sin_theta;
      added_pt.intensity = pl_orig.points[i].intensity;
      added_pt.curvature =
...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  for (uint i = 0; i < plsize; ++i)
  {
    ...
    added_pt.x = pl_orig.points[i].x * cos_theta - pl_orig.points[i].z * sin_theta;
    added_pt.y = pl_orig.points[i].y;
    added_pt.z = pl_orig.points[i].x * sin_theta + pl_orig.points[i].z * cos_theta;
